...
> **TODO（実装フェーズ未割当の設計仕様 / code-review 検出）**
>
> overview.md §3.6 は撮影直前の **ページ安定待機**（`document.getAnimations()` と `MutationObserver` によるポーリングで DOM/アニメーションの静止を待つ機構）を必須仕様として定義しているが、**この機構を実装するフェーズがどの phase-N-*.md のスコープにも入っていない**。本フェーズ（アクション instrumentation）はその実装先の第一候補だが、現状の §2 スコープには連番撮影・前後撮影・手動 screenshot・ファイル名生成のみが含まれ、安定待機は含まれていない。
>
> 対応方針（本フェーズ着手時に判断）:
> - ページ安定待機を本フェーズのスコープに取り込む（§2/§5/§6 を更新）か、専用フェーズに切り出すかを決める。
> - overview.md §3.6 がポーリング間隔・最大試行回数・除外アニメーション名・タイムアウト時挙動を「configure で上書き可能にする（§11）」「実装時に決定（§11）」と参照しているが、§11 の決定事項リストにこれらが無い。取り込むフェーズが決まったら、対応する決定事項を overview.md §11 に追記し、設定項目は phase-5-configuration.md のスコープに反映する。
