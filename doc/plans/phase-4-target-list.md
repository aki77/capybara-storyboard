...
> **TODO（本フェーズ着手時に確定させる未解決事項 / code-review 検出）**
>
> 上記2ケースの区別は、テストケースを用意するだけでは成立しない。overview.md §3.4 の `TargetListPolicy` は「正規化済みパス集合を受け取り、空なら true（絞り込みなし=全撮影）を返す」ピュアな集合演算として定義されており、**「未指定で空集合」なのか「空ファイル指定で空集合」なのかを区別する入力が仕様上どこにも定義されていない**。このままだと §6 が要求する「空ファイル指定→撮影0件」を満たせず、§2/§6 の受け入れ条件（空集合→全撮影）と正面から矛盾する。
>
> 本フェーズ着手時に、区別をどのコンポーネントが担うかを決めて overview.md §3.4/§6 と本ドキュメント §2/§6 を整合させること。取りうる案:
> - **未指定時は `TargetListPolicy` を合成しない**（デフォルトポリシー構築側で分岐し、空ファイル指定時のみ空集合の `TargetListPolicy` を構築して常に false を返させる）。
> - `TargetListPolicy` に「明示指定されたか」のフラグ/センチネルを渡し、Policy 自身が「空集合かつ明示指定あり→false」を判定する。
>
> どちらを採るにせよ、`TargetListPolicy` の責務（§4）と受け入れ条件（§6）の記述をこの決定に合わせて更新する。
