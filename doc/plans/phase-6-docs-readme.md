...
導入者（gem を新規に使い始める人）向けに、インストールから有効化までを一気通貫で読める README を整備する。対応する仕様は overview.md §5（有効化モデル）。gist の `AutoScreenshots` concern からの移行手順（overview.md §9）は本フェーズのスコープ外とし、純粋な新規導入者向けの README とする。

## 2. スコープ

### 含むもの

- gem のインストール手順（`bundle add capybara-storyboard` のみを案内する）。
...
- gist の `AutoScreenshots` concern からの移行手順は本フェーズでは扱わない（overview.md §9 の仕様自体は将来のために残すが、README には書かない）。
- インストール手順は `bundle add capybara-storyboard` のみを案内し、Gemfile への git 参照によるインストール手順は書かない。
- CHANGELOG.md は本フェーズでは更新しない。
...
- **インストール手順**: `bundle add capybara-storyboard` のみを案内する（Gemfile への git 参照によるインストール手順は書かない）。RSpec 側の読み込み方法（`spec_helper.rb` / `rails_helper.rb` への require の記載）。
...
- **レビュー観点**: overview.md の該当節（§5）と記述内容が矛盾していないか、章立てが README として自然な導線（インストール → 使い方 → 有効化モデル → 前提の割り切り）になっているかを確認する。
...
- overview.md §9（gist からの移行、本フェーズでは非対象）
